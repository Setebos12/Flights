export default function FlightCard({ flight, user, onBook }) {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-4 flex items-center gap-4">
      ...
      <div className="flex flex-col gap-1 items-end shrink-0">
        ...
        {(flight.availableSeats != null || flight.seatCount != null) && (
          <span className="text-xs text-green-700 bg-green-50 px-2 py-1 rounded-full">
            {flight.availableSeats != null
              ? `${flight.availableSeats} seats left`
              : `${Math.max(0, (flight.seatCount || 0) - (flight.bookedSeatsCount || 0))} seats left`}
          </span>
        )}
      </div>

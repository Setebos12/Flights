...
public interface ReservationRepository extends JpaRepository<Reservation, Long> {
    ...
    Long createReservation(
            ...
            @Param("p_calculated_price") BigDecimal paymentAmount,
            @Param("p_currency_code") String currencyCode
    );
    ...
    @Query(value =
        "SELECT r.id, ps.description as status " +
        "FROM reservations r " +
        "LEFT JOIN payments p ON r.id = p.reservations_id " +
        "LEFT JOIN payment_status ps ON p.payment_status_id = ps.payment_status_id " +
        "WHERE r.id IN :reservationIds",
        nativeQuery = true)
    List<Object[]> findReservationStatusesRaw(
